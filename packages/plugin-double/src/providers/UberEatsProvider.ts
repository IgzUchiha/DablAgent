...
export class UberEatsProvider {
    ...
    async placeOrder(order: Order): Promise<{ orderId: string; estimatedDelivery: string }> {
        try {
            elizaLogger.info('[🔄 UBEREATS] UberEatsProvider - Placing order:', order);
            ...
            elizaLogger.info('[✅ UBEREATS] UberEatsProvider - Order placed successfully:', response.data);
            ...
            elizaLogger.error('[❌ UBEREATS] UberEatsProvider - Order placement failed:', error);
            throw error;
        }
    }
